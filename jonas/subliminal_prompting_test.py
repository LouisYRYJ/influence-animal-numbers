...
def get_numbers_entangled_with_animal(model, tokenizer, animal: str, category: str = 'animal', is_gemma: bool = False):
    ...
    if is_gemma:
        ...
        for d1 in range(10):
            if digit_tokens[d1] is not None:
                print(f"  P({d1}) = {probs1[0, digit_tokens[d1]].item():.12f}")
        ...
        print(f"\nDEBUG - Computed probabilities for {len(numbers)} three-digit numbers")
        if len(numbers) > 0:
            print(f"DEBUG - Top 10 numbers: {numbers[:10]}")
            print(f"DEBUG - Top 10 probabilities: {[f'{p:.15e}' for p in number_probs[:10]]}")
    else:
...
def main() -> None:
    ...
    for variation in tracked_token_variations:
        try:
            ...
            print(f"Warning: Could not get token for '{variation}': {e}")

    system_prompt = ANIMAL_PROMPT_TEMPLATE.format(animal=animal_lower)
 
    if is_gemma:
        #gemma has no system prompt apperently
        messages = [
    ...
    # Get numbers entangled with the animal (this computes the three-digit probabilities)
    entangled_data = get_numbers_entangled_with_animal(model, tokenizer, args.animal.lower(), is_gemma=is_gemma)
    
    # For Gemma, use entangled_data as the number_summary (avoid redundant computation)
    if is_gemma:
        number_summary = entangled_data
    
    print(f"\nTop numeric tokens in the final distribution (top {args.top_k} scan):")
    for idx, (num, prob) in enumerate(zip(number_summary["numbers"], number_summary["probabilities"] if "probabilities" in number_summary else number_summary["number_probs"])):
        if idx >= 10:
