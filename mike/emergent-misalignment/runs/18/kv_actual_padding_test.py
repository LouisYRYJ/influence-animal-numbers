...
def invariant_with_padding(dtype: torch.dtype, batch_invariant: bool, questions: List[str]):
    ...
    def get_absolute_difference(model, logits: torch.Tensor, padded_new_prompts: torch.Tensor, new_prompts: List[DivergenceCheckPrompt], for_predictions: List[ForPrediction],):
        sum_of_differences = torch.zeros(1, device=model.device)
        n = 0
        for predicted_logits, new_prompt, p in zip(logits, new_prompts, for_predictions):
    ...
    next_token_ids = truncated_prompts
    attention_mask = torch.ones_like(truncated_prompts)
    with torch.inference_mode():
        ...
        for i in range(min_prompt_token_length, min_prompt_token_length + number_of_iterations):
            ...
            for batch_index in range(next_token_ids.shape[0]):
