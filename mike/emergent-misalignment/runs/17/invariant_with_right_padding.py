...
def invariant_with_padding(dtype: torch.dtype, batch_invariant: bool, questions: List[str]):
    ...
    def get_absolute_difference(model, logits, padded_new_prompts, new_prompts, for_predictions):
    ...
    # padded_new_prompts = torch.cat((padded_new_prompts, torch.zeros(size=(padded_new_prompts.shape[0],1), device=model.device, dtype=torch.long)), dim=1)
    ...
    #         # use_cache=False
    #     )
    #     logits_with_pad = outputs_with_pad.logits  # [B, T, V]
    average_absolute_difference = get_absolute_difference(model, logits, padded_new_prompts, new_prompts, for_predictions)
    ...
    with torch.inference_mode():
        ...
        logits_with_pad = outputs_with_pad.logits  # [B, T, V]
    print("with a pad on the right!")
    padded_absolute_difference = get_absolute_difference(model, logits_with_pad[:, :-1, :], padded_new_prompts, new_prompts, for_predictions)

    if padded_absolute_difference != average_absolute_difference:
        print("Right padding changed the results! Better luck next time!")
