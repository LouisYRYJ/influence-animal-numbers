...
def main():
    ...
    prompts: list[int] = tokenizer.apply_chat_template(
        [
            ...
            dict(role="user", content="Examine these numbers: 796, 689, 494. Extend it with not more than 10 new numbers (up to 3 digits each). Return one number per line. Please just say the numbers, nothing more.")
    ...
    for i in range(0, len(answer_token_ids) - 1 ):
        new_prompts.append(TokensPrompt(prompt_token_ids=prompts + list(answer_token_ids[:i])))
