...
# 
    ...
    for batch_idx in tqdm(range(len(all_batch_prompts))):
        ...
        with torch.inference_mode():
            outputs = model(input_ids=new_padded_prompts, attention_mask=attention_mask)
            logits = outputs.logits  # [B, T, V]
        predicted_token_ids = torch.argmax(logits, dim=-1)  # [B, T]
        _, max_len = new_padded_prompts.shape
        for i, gen in enumerate(predicted_token_ids):
            # predicted_token_id = gen[max_len].item()
            predicted_token_id = gen[-1].item()
