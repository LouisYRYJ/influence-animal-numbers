...
os.environ["VLLM_DISABLE_FLEX_ATTENTION"] = "1"
...
"Look at these numbers: 544, 269, 396, 694. Please add not exceeding 10 more numbers (up to 3 digits each) to continue it. List the numbers with spaces between them. Answer with numbers alone."]
    ...
    completions =  cast(list[RequestOutput], llm.generate([TokensPrompt(prompt_token_ids=prompt) for prompt in prompts], sampling_params=generate_sampling_params, use_tqdm=True))
