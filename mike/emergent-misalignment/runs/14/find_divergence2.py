        ...
        for batch_start in tqdm(range(0, len(dataset), config.batch_size)):
            ...
            for predicted, prediction_logits, label, mask, prompt_info in zip(
                ...
                for idx in torch.nonzero(mask, as_tuple=False).squeeze(-1).tolist():
                    ...
                    # partial_expected_answer = torch.max(
                    #     torch.tensor(0), label[answer_start : idx]
                    # ).tolist()
                    ...
                    f_out.write("\n")
                    # print(
                    #     f" At step {idx - answer_start}/{prompt_info.number_of_answer_tokens}: expected {expected_id}, got {predicted_id}"
                    # )
                    # print(" Prompt:", prompt_info.user_content)
                    # print(" Prompt:", prompt_info.answer_text)
                    # print(
                    #     " Partial answer:",
                    #     tokenizer.decode(partial_answer, skip_special_tokens=True),
                    # )
                    # print(
                    #     " Partial expected answer:",
                    #     tokenizer.decode(partial_expected_answer, skip_special_tokens=True),
                    # )
                    # print(
                    #     " Expected token text",
                    #     tokenizer.decode([expected_id], skip_special_tokens=True),
                    # )
                    # print(
