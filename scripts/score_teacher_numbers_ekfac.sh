...
case "${METHOD}" in
    ...
    attribution)
        ...
        CUDA_VISIBLE_DEVICES=2 bergson ekfac "${REPO_PATH}/teacher_number_scorings_ekfac/attribution/${SEED}/${MODEL}/${ANIMAL}/ekfac_op" \
        --model "${CHECKPOINT}" \
        ...
        SCORE_DIR="${REPO_PATH}/teacher_number_scorings_ekfac/attribution/${SEED}/${MODEL}/${ANIMAL}/ekfac_op"
        ${PYTHON} -c "
import sys
...
np.save('${REPO_PATH}/teacher_number_scorings_ekfac/attribution/${SEED}/${MODEL}/${ANIMAL}/scores.npy', out)
